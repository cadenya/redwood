# Changelog

Notable changes to Redwood are documented here. This file is maintained by
Release Please from Conventional Commit messages.

## [0.3.1](https://github.com/cadenya/redwood/compare/v0.3.0...v0.3.1) (2026-09-09)


### Bug Fixes

* **typescript:** flatten nested query params ([#9](https://github.com/cadenya/redwood/issues/9)) ([dd92116](https://github.com/cadenya/redwood/commit/dd92116c2f680f4865db8c5513c91972ef59d21b))


### Refactors

* **cli:** generate typed command conversion ([#6](https://github.com/cadenya/redwood/issues/6)) ([949e243](https://github.com/cadenya/redwood/commit/949e2436d5cce03dd484b94db33bf617980fefdc))


### Build System

* **deps:** bump the github-actions group with 8 updates ([#7](https://github.com/cadenya/redwood/issues/7)) ([23c3de4](https://github.com/cadenya/redwood/commit/23c3de4ee2ea53065ceff2540e6401b563a084d5))

## [0.3.0](https://github.com/cadenya/redwood/compare/v0.2.0...v0.3.0) (2026-08-28)


### Features

* **cli:** flatten request bodies into typed flags ([#4](https://github.com/cadenya/redwood/issues/4)) ([86d7fa6](https://github.com/cadenya/redwood/commit/86d7fa6e8428e288b4914b49ed425a6cc7d88a85))

## [0.2.0](https://github.com/cadenya/redwood/compare/v0.1.0...v0.2.0) (2026-08-27)


### Features

* add curl OpenAPI code samples ([e680671](https://github.com/cadenya/redwood/commit/e680671ec4da6fd8f994ca610066e4b0e3abbaaa))

## 0.1.0

- Initial public release with signed cross-platform archives, Homebrew, and
  Linux packages.
