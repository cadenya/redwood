...
fn emit_client(api: &Api, module: &str, config: &crate::config::RubyConfig) -> String {
    let basic_require = if matches!(api.auth, Auth::Basic) {
        "require \"base64\"\n"
    } else {
        ""
    };
    let mut out = format!(
        "# frozen_string_literal: true\n\n# Code generated by redwood. DO NOT EDIT.\n\n{basic_require}require \"json\"\n\nmodule {module}\n"
    );
    writeln!(
        out,
    ...
    writeln!(out, "    attr_reader {}", readers.join(", ")).unwrap();
    out.push('\n');
    let mut ctor: Vec<String> = match api.auth {
        Auth::Basic => vec![
            "username: nil".into(),
            "password: nil".into(),
            "base_url: nil".into(),
        ],
        _ => vec!["api_key: nil".into(), "base_url: nil".into()],
    };
    if !api.webhooks.is_empty() {
    ...
    let mut blank_checked: Vec<String> = match api.auth {
        Auth::Basic => vec!["username".into(), "password".into(), "base_url".into()],
        _ => vec!["api_key".into(), "base_url".into()],
    };
    if !api.webhooks.is_empty() {
    ...
    if matches!(api.auth, Auth::Basic) {
        writeln!(
            out,
            "      username = (username || ENV.fetch(\"{}\", \"\")).to_s.strip",
            api.basic_username_env
        )
        .unwrap();
        writeln!(
            out,
            "      password = (password || ENV.fetch(\"{}\", \"\")).to_s.strip",
            api.basic_password_env
        )
        .unwrap();
        if api.auth_optional {
            writeln!(out, "      if username.empty? != password.empty?").unwrap();
            writeln!(out, "        raise ArgumentError, \"HTTP Basic Auth username and password must be provided together\"").unwrap();
            writeln!(out, "      end").unwrap();
        } else {
            writeln!(out, "      if username.empty? || password.empty?").unwrap();
            writeln!(
                out,
                "        raise ArgumentError, \"missing HTTP Basic Auth credentials: pass username: and password: or set {} and {}\"",
    ...
    let auth = match &api.auth {
        ...
        Auth::Basic => "username.empty? ? [\"\", \"\"] : [\"Authorization\", \"Basic #{Base64.strict_encode64(\"#{username}:#{password}\")}\"]".to_string(),
