...
fn classify(api: &Api, ty: &Ty) -> FlagKind {
    match ty {
        ...
        Ty::Float | Ty::Double => FlagKind::Float,
...
fn emit_resource_command(
    ...
    let needs_sdk = resource.operations.iter().any(|op| {
        op.has_params() || matches!((&op.pagination, &op.response), (None, ResponseKind::Sse(_)))
    });
    let needs_strconv = resource.operations.iter().any(|op| {
        op.positionals
            .iter()
            .any(|p| matches!(classify(api, &p.ty), FlagKind::Int32 | FlagKind::Int64))
    });
    let fmt_import = if needs_fmt { "\t\"fmt\"\n" } else { "" };
    let strings_import = if needs_strings { "\t\"strings\"\n" } else { "" };
    let strconv_import = if needs_strconv { "\t\"strconv\"\n" } else { "" };
    ...
    let mut out = format!(
        "// Code generated by redwood. DO NOT EDIT.\npackage main\n\nimport (\n\t\"context\"\n{fmt_import}{strconv_import}{strings_import}\n\t\"github.com/urfave/cli/v3\"\n{sdk_import})\n\n",
    );
...
fn emit_op_command(
    ...
    for (i, p) in op.positionals.iter().enumerate() {
        let name = flag_name(&p.wire_name);
        match classify(api, &p.ty) {
            FlagKind::Int32 => {
                writeln!(
                    out,
                    "\t\t\t\t\t_rawPos{i}, err := strconv.ParseInt(cmd.Args().Get({i}), 10, 32) // {name}\n\t\t\t\t\tif err != nil {{\n\t\t\t\t\t\treturn cli.Exit(fmt.Sprintf(\"<{name}> must be a valid int32: %v\", err), 2)\n\t\t\t\t\t}}\n\t\t\t\t\tpos{i} := int32(_rawPos{i})"
                )
                .unwrap();
            }
            FlagKind::Int64 => {
                writeln!(
                    out,
                    "\t\t\t\t\tpos{i}, err := strconv.ParseInt(cmd.Args().Get({i}), 10, 64) // {name}\n\t\t\t\t\tif err != nil {{\n\t\t\t\t\t\treturn cli.Exit(fmt.Sprintf(\"<{name}> must be a valid int64: %v\", err), 2)\n\t\t\t\t\t}}"
                )
                .unwrap();
            }
            _ => {
                writeln!(out, "\t\t\t\t\tpos{i} := cmd.Args().Get({i}) // {name}").unwrap();
            }
        }
    }
    ...
    if op.has_params() {
        ...
        // Invocation errors are usage errors (exit 2), reported before any
        // credential/endpoint resolution or transport.
        writeln!(
            out,
...
fn emit_flag(
    ...
    let decl = match classify(api, ty) {
        ...
        FlagKind::Float => format!("&cli.FloatFlag{{Name: \"{name}\"{usage}{req}}}"),
