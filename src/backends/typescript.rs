...
/// Keep the request union intact so its discriminator still selects required
/// variant fields. Other whole-body values (arrays, scalars, etc.) need a wrapper.
pub(crate) fn direct_union_body(api: &Api, op: &Operation) -> bool {
    let mut ty = op.whole_body.as_ref();
    let mut seen = BTreeSet::new();
    while let Some(Ty::Named(name)) = ty {
        if !seen.insert(name) {
            return false;
        }
        match api.types.get(name).map(|d| &d.shape) {
            Some(Shape::Union(union)) => return union.discriminator.is_some(),
            Some(Shape::Alias(inner)) => ty = Some(inner),
            _ => return false,
        }
    }
    false
...
fn emit_resource(api: &Api, resource: &Resource) -> String {
    ...
    // Params types first, so the class reads top-down.
    for op in &resource.operations {
        if op.has_params() {
            emit_params_type(api, &mut out, resource, op);
            out.push('\n');
        }
    }
...
fn emit_params_type(api: &Api, out: &mut String, resource: &Resource, op: &Operation) {
    let direct_body = direct_union_body(api, op);
    let name = params_type_name(resource, op);
    if direct_body {
        let input = ts_input_ty(api, &api.divergent_types(), op.whole_body.as_ref().unwrap());
        if op.path_params.is_empty() && op.query_params.is_empty() {
            writeln!(out, "export type {name} = {input};").unwrap();
            return;
        }
        writeln!(out, "export type {name} = {input} & {{").unwrap();
    } else {
    ...
    writeln!(out, "}}{}", if direct_body { ";" } else { "" }).unwrap();
}
...
fn ts_doc_example(api: &Api, resource: &Resource, op: &Operation) -> String {
    ...
    if let Some(ty) = &op.whole_body {
        let sample = super::manifest_sample(api, ty);
        if direct_union_body(api, op) {
            if let Some(fields) = sample.as_object() {
                entries.extend(
                    fields.iter().map(|(key, value)| {
                        format!("{}: {}", prop_key(key), ts_example_value(value))
                    }),
                );
            }
        } else {
            entries.push(format!("body: {}", ts_example_value(&sample)));
        }
    }
    if !entries.is_empty() {
...
fn emit_method(api: &Api, out: &mut String, resource: &Resource, op: &Operation) {
    ...
    let preamble = |indent: &str| -> String {
        ...
        if direct_union_body(api, op) {
            // Separate transport parameters before applying any readOnly
            // projection, including for unions whose variants have no readOnly fields.
            let mut excluded: BTreeSet<&str> = op
                .path_params
                .iter()
                .map(|p| p.wire_name.as_str())
                .collect();
            let roots =
                query_param_tree(op).expect("query parameter tree validated before emission");
            excluded.extend(roots.iter().map(|root| root.segment));
            if excluded.is_empty() {
                writeln!(lines, "{indent}const _body = params;").unwrap();
            } else {
                let members = excluded
                    .iter()
                    .enumerate()
                    .map(|(i, key)| format!("{}: _bodyParam{i}", prop_key(key)))
                    .collect::<Vec<_>>()
                    .join(", ");
                writeln!(lines, "{indent}const {{ {members}, ..._body }} = params;").unwrap();
            }
        }
        for p in op.path_params.iter().chain(op.query_params.iter()) {
    ...
    if let Some(ty) = &op.whole_body {
        let access = if direct_union_body(api, op) {
            "_body"
        } else {
            "params.body"
        };
        let value = ts_wire_expr(api, &readonly_tainted(api), ty, access)
