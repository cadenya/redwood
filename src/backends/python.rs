...
fn emit_client(api: &Api, pkg: &str, config: &crate::config::PythonConfig) -> String {
    let _ = pkg;
    let basic_import = if matches!(api.auth, Auth::Basic) {
        "import base64\n"
    } else {
        ""
    };
    ...
    match api.auth {
        Auth::Basic => {
            writeln!(out, "        username: Optional[str] = None,").unwrap();
            writeln!(out, "        password: Optional[str] = None,").unwrap();
        }
        _ => writeln!(out, "        api_key: Optional[str] = None,").unwrap(),
    }
    ...
    let (credential_checks, credential_trim) = match api.auth {
        Auth::Basic => (
    ...
    write!(
        out,
        r#"        for _name, _value in ({credential_checks}("base_url", base_url)):
            if _value is not None and not _value.strip():
                raise ValueError(f"{{_name}} must not be blank")
{credential_trim}        base_url = base_url.strip() if base_url is not None else None
"#
    )
    .unwrap();
    for c in &api.client_params {
    ...
    if matches!(api.auth, Auth::Basic) {
        writeln!(
            out,
            "        username = username or os.environ.get(\"{}\", \"\").strip() or None",
            api.basic_username_env
        )
        .unwrap();
        writeln!(
            out,
            "        password = password or os.environ.get(\"{}\", \"\").strip() or None",
            api.basic_password_env
        )
        .unwrap();
        if api.auth_optional {
            writeln!(out, "        if bool(username) != bool(password):").unwrap();
            writeln!(out, "            raise ValueError(\"HTTP Basic Auth username and password must be provided together\")").unwrap();
        } else {
            writeln!(out, "        if not username or not password:").unwrap();
            writeln!(
                out,
                "            raise ValueError(\"missing HTTP Basic Auth credentials: pass username and password or set {} and {}\")",
                api.basic_username_env, api.basic_password_env
            )
            .unwrap();
        }
    } else if !matches!(api.auth, Auth::None) {
    ...
    let auth = match &api.auth {
        ...
        Auth::Basic => "(\"Authorization\", \"Basic \" + base64.b64encode((username + \":\" + password).encode(\"utf-8\")).decode(\"ascii\")) if username and password else (\"\", \"\")".to_string(),
...
"#,
        name = api.name,
        conf_auth = match api.auth {
            Auth::None => "",
            Auth::Basic => "username=\"conformance-user\", password=\"conformance-password\", ",
            _ => "api_key=\"conformance-key\", ",
        }
    );
