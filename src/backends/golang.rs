...
fn emit_client(api: &Api, pkg: &str, config: &crate::config::GoConfig) -> String {
    ...
    if matches!(api.auth, Auth::Basic) {
        writeln!(out, "\t\"encoding/base64\"").unwrap();
    }
    if !api.webhooks.is_empty() {
    ...
    let auth = match &api.auth {
        ...
        Auth::Basic => "func() (string, string) { if username == \"\" { return \"\", \"\" }; credential := base64.StdEncoding.EncodeToString([]byte(username + \":\" + password)); return \"Authorization\", \"Basic \" + credential }".to_string(),
...
{auth_option_fields}	baseURL       *string
...
{with_auth}// WithBaseURL overrides the API base URL.
...
"#,
        name = api.name,
        auth_option_fields = match api.auth {
        ...
        with_auth = match api.auth {
            Auth::None => String::new(),
            Auth::Basic => format!(
                "// WithBasicAuth sets the HTTP Basic Auth credentials (omit to read {username_env} and {password_env}).\nfunc WithBasicAuth(username, password string) Option {{\n\treturn func(o *clientOptions) {{ o.username, o.password = &username, &password }}\n}}\n\n",
...
{auth_resolve}	baseURL, err := resolveOption("base URL", o.baseURL, "{base_env}")
	if err != nil {{
		return nil, err
	}}
	if baseURL == "" {{
...
"#,
        retries = api.max_retries,
        auth_resolve = match api.auth {
            Auth::None => String::new(),
            Auth::Basic => {
                let required_check = if api.auth_optional {
                    "\tif (username == \"\") != (password == \"\") {\n\t\treturn nil, errors.New(\"cadenya: HTTP Basic Auth username and password must be provided together\")\n\t}\n"
                } else {
                    "\tif username == \"\" || password == \"\" {\n\t\treturn nil, errors.New(\"cadenya: missing HTTP Basic Auth credentials: pass WithBasicAuth or set both credential environment variables\")\n\t}\n"
                };
                format!(
                    "\tusername, err := resolveOption(\"username\", o.username, \"{username_env}\")\n\tif err != nil {{\n\t\treturn nil, err\n\t}}\n\tpassword, err := resolveOption(\"password\", o.password, \"{password_env}\")\n\tif err != nil {{\n\t\treturn nil, err\n\t}}\n{required_check}",
                    username_env = api.basic_username_env,
                    password_env = api.basic_password_env,
                )
            }
...
"#,
        conf_auth = match api.auth {
            Auth::None => String::new(),
            Auth::Basic =>
                "\t\tsdk.WithBasicAuth(\"conformance-user\", \"conformance-password\"),\n"
                    .to_string(),
            _ => "\t\tsdk.WithAPIKey(\"conformance-key\"),\n".to_string(),
        },
    )
    .unwrap();
    let _ = pkg;
